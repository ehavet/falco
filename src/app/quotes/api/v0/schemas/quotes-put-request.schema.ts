...
export const quotePutRequestBodySchema: Joi.ObjectSchema = Joi.object({
  ...
  risk: Joi.object({
    property: Joi.object({
      ...
      postal_code: Joi.string().required().allow(null).description('Property postal code').example('95470'),
  ...
  policy_holder: Joi.object({
    email: Joi.string().email().required().allow(null).description('Contact email').example('john.doe@email.com'),
    phone_number: Joi.string().required().allow(null).max(15).description('Contact phone number').example('+33684205510')
