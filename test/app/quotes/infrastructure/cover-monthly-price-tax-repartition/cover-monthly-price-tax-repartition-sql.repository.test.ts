...
describe('Quotes - Infra - CoverMonthlyPriceTaxRepartitionSql Repository', async () => {
  ...
  describe('#getAllWithoutZone', async () => {
    afterEach(async () => {
      await PricingMatrixSqlModel.destroy({ where: { partner: partnerCode } })
    })
    ...
    it('should return price tax repartition for given partner, room count and pricing zone', async () => {
      // Given
      const pricingMatrixFixture = [
        { partner: partnerCode, roomCount: 1, cover: 'DDEAUX', coverMonthlyPrice: '1.81333', pricingZone: 'ZNOTFOUND', coverMonthlyPriceExclTax: '1.00000', coverMonthlyPriceTax: '0.81333' },
        { partner: partnerCode, roomCount: 1, cover: 'INCEND', coverMonthlyPrice: '1.16750', pricingZone: 'ZNOTFOUND', coverMonthlyPriceExclTax: '1.00000', coverMonthlyPriceTax: '0.16750' },
        { partner: partnerCode, roomCount: 1, cover: 'VOLXXX', coverMonthlyPrice: '0.84417', pricingZone: 'ZNOTFOUND', coverMonthlyPriceExclTax: '0.80000', coverMonthlyPriceTax: '0.04417' }
      ]
      await PricingMatrixSqlModel.bulkCreate(pricingMatrixFixture)

      // When
      const coverMonthlyPriceTaxRepartitions = await coverMonthlyPriceTaxRepartitionSqlRepository.getAllWithoutZone(partnerCode, 1)
    ...
    it('should throw an error if there is no cover monthly price tax repartitions for the given partner', async () => {
      ...
      await PricingMatrixSqlModel.bulkCreate(pricingMatrixFixture)
      // When
      const promise = coverMonthlyPriceTaxRepartitionSqlRepository.getAllWithoutZone('myOtherPartner', 1)
    ...
    it('should throw an error if there is multiple monthly prices for one cover', async () => {
      ...
      // When
      const promise = coverMonthlyPriceTaxRepartitionSqlRepository.getAllWithoutZone(partnerCode, 1)
